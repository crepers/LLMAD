...
prompt_template_Machine = """
## Instructions
Determine if there are any anomalies in the provided Machine data sequence.
The data consists of time series values (`param1`) recorded at specific timestamps (`Datetime`).
{data_description}
...
- Historical normal data sequence: `{normal_data}`
- Historical anomaly data sequence(*XXX* is anomaly point), `{anomaly_data}`
- The latest `{data_len}` data points for evaluation:
{data}
"""
...
class PromptTemplate:
    ...
    def get_template_5(self, normal_data, data, data_len, anomaly_datas, data_description=""):
        anomaly = ''
        for idx, anomaly_data in enumerate(anomaly_datas):
            anomaly += 'sequence %d: %s\n' % (idx+1, anomaly_data)
        prompt_res = self.prompt_template.replace('{normal_data}', normal_data).replace('{data}', data).replace('{data_len}', str(data_len)).replace('{anomaly_data}', anomaly).replace('{data_description}', data_description)
        return prompt_res
